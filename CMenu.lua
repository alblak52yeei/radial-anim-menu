...
local div = Vector2( scr.x / 1920, scr.y / 1080 )
div = ( div.x + div.y ) / 2

local pFont = DxFont( 'Resources/JosefinSans.ttf', 13 * div, false, 'antialiased' )

local SVG_DATA = {
    { 
        name        = 'Pencil',
        circle_pos  = { 650, 350, 150, 150 },
        icon_pos    = { 695, 390, 950, 950 },
        text_pos    = { 1450, 460 },
        data        = 
        [[<svg>
    ...
    { 
        ...
        circle_pos  = { 850, 290, 150, 150 },
        icon_pos    = { 896, 332, 950, 950 },
        text_pos    = { 1850, 400 },
        data = 
        [[<svg>
    ...
    { 
        name = 'Tools',
        circle_pos  = { 990, 450, 150, 150 },
        icon_pos    = { 1035, 492, 950, 950 },
        text_pos    = { 2130, 560 },
        data = 
        [[<svg>
    ...
    { 
        ...
        circle_pos  = { 850, 620, 150, 150 },
        icon_pos    = { 898, 665, 950, 950 },
        text_pos    = { 1850, 730 },
        data = 
        [[<svg>
    ...
    { 
        name = 'Brush',
        circle_pos  = { 650, 550, 150, 150 },
        icon_pos    = { 703, 595, 950, 950 },
        text_pos    = { 1450, 655 },
        data = 
        [[<svg>
...
for i, v in pairs( SVG_DATA ) do
    aSVG[ v.name ] = svgCreate( 1500, 1500, v.data, function( bLoaded )
        if not bLoaded then return end

        -- Все ли SVG прогружены?
        if i == #SVG_DATA then
...
function render()
    for _, v in ipairs( SVG_DATA ) do
        -- Is circle?
        if not v.circle_pos then return end

        if not aHoverData[ v.name ] then
            aHoverData[ v.name ] = { }
        end

        dxDrawImage( v.circle_pos[ 1 ] * div, v.circle_pos[ 2 ] * div, v.circle_pos[ 3 ] * div, v.circle_pos[ 4 ] * div, aSVG[ 'Circle' ], 0, 0, 0, 0xFFFFFFFF, false )

        if IsMouseInPosition( v.circle_pos[ 1 ] * div, v.circle_pos[ 2 ] * div, v.circle_pos[ 3 ] * div, v.circle_pos[ 4 ] * div ) then
            if not aHoverData[ v.name ].hover then
                aHoverData[ v.name ].startTime   = getTickCount( )
                aHoverData[ v.name ].endTime     = aHoverData[ v.name ].startTime + 200
            end

            local elapsedTime   = getTickCount( ) - aHoverData[ v.name ].startTime
            local duration      = aHoverData[ v.name ].endTime - aHoverData[ v.name ].startTime
            local progress      = elapsedTime / duration

            Y = interpolateBetween( 
                v.icon_pos[ 2 ] + 5, 0, 0,
                v.icon_pos[ 2 ], 0, 0, 
                progress, "Linear")

            dxDrawImage( v.icon_pos[ 1 ] * div, Y * div, v.icon_pos[ 3 ] * div, v.icon_pos[ 4 ] * div, aSVG[ v.name ], 0, 0, 0, 0xFFFFFFFF, false )


            aHoverData[ v.name ].hover = true

            X, Y, size = interpolateBetween( 
                v.circle_pos[ 1 ] - 20,  v.circle_pos[ 2 ] - 20, 170,
                v.circle_pos[ 1 ] - 30, v.circle_pos[ 2 ] - 30,     190, 
                progress, "Linear")

            dxDrawImage( X * div + 9 * div, Y * div + 11 * div, size * div, size * div, 'Resources/circle.png', 0, 0, 0, 0xFFFFFFFF, false )

            -- Text
            Y, alpha, size = interpolateBetween( 
                v.text_pos[ 2 ] - 25, 0, 0,
                v.text_pos[ 2 ] + 5, 255, 1, 
                progress, "Linear" )

            dxDrawText( v.name, v.text_pos[ 1 ] * div, Y * div, 0, 0, tocolor( 0, 0, 0, alpha ), size, pFont, 'center' )
        else

            if aHoverData[ v.name ].hover then
                aHoverData[ v.name ].startTime  = getTickCount( )
                aHoverData[ v.name ].endTime    = aHoverData[ v.name ].startTime + 200
            end

            if aHoverData[ v.name ].startTime and aHoverData[ v.name ].endTime then
                local elapsedTime = getTickCount( ) - aHoverData[ v.name ].startTime
                local duration = aHoverData[ v.name ].endTime - aHoverData[ v.name ].startTime
                ...
                dxDrawImage( v.icon_pos[ 1 ] * div, ( v.icon_pos[ 2 ] * div + 5 * div ), v.icon_pos[ 3 ] * div, v.icon_pos[ 4 ] * div, aSVG[ v.name ], 0, 0, 0, 0xFFFFFFFF, false )
            end
